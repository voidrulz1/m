...
var BOT_PASSWORD = "luvneverdie"; // Password
//var BOT_ID = "trulymadly"; // Bot ID
//var BOT_PASSWORD = "Zxcv1234."; // Password
...
var Client = /** @class */ (function() {
  ...
  Client.prototype.sendSong = async function(searchQuery, roomName) {
    ...
    try {
      ...
      var newArray;
      newArray = api1Response.items.filter(items => items.d != '🔴 Live')
      ////console.log(newArray);
      vidID = newArray[0].v;
      vidTitle = newArray[0].t;
      vidTime = newArray[0].d;

      const ytURL = "https://youtu.be/" + vidID;
      const apiConvert = await fetch("https://yt5s.io/api/ajaxSearch", {
        "headers": {
          "accept": "*/*",
          "accept-language": "en-US,en;q=0.9,hi;q=0.8",
          "content-type": "application/x-www-form-urlencoded; charset=UTF-8",
          ...
          "x-requested-with": "XMLHttpRequest",
          "cookie": "__atssc=google%3B12; __atuvc=9%7C48%2C35%7C49; __atuvs=6391e48b6098eeb4000",
          "Referer": "https://yt5s.io/en3/youtube-to-mp3?q=https%3A%2F%2Fwww.youtube.com%2Fwatch%3Fv%3D" + vidID,
          "Referrer-Policy": "strict-origin-when-cross-origin"
        },
      ...
      const getLink = await fetch("https://backend.svcenter.xyz/api/convert-by-45fc4be8916916ba3b8d61dd6e0d6994", {
        ...
        "body": "v_id=" + vidID + "&ftype=mp3&fquality=64&token=" + vidToken + "&timeExpire=" + vidTimeExpired + "&client=yt5s.io",
        "method": "POST"
      });
      ...
      if (this.myRole != "none") {
        ...
        this.sendRoomMsg(roomName, `Music Title : ${vidTitle}`);
        this.sendRoomMsg(roomName, "", "", dlink);
      }
      var myObj = {
  ...
  Client.prototype.sendRoomMsg = async function(roomName, msg, photoUrl, audioUrl) {
